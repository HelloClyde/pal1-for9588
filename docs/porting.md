...
- `src/libc`：无操作系统 C 运行时所需的最小 libc，以及单资源包的虚拟文件层。
...
只读商业资源集中在 `PAL9588.PAK`。`src/libc/runtime.c` 的 `access` 和 stdio 包装层
先查询 PAK 目录，再回退到普通 NAND 文件；存档、配置和日志仍走普通可写文件。
容器不在设备端解包，一个共享底层句柄配合 32 个静态虚拟 `FILE` 槽支持 MKF、文本
和 AVI 交错随机读取。静态槽避免固件小块堆分配在启动阶段失败，单句柄也避免同时打开
大量资源耗尽固件句柄表。详细布局见 `docs/palpak-format.md`。
...
传输丢包为 0。存档两次启动读回和
代表性战斗胜利也已跑通。新增的单包路径在隔离 NAND 只保留
`PAL9588.PAK` 时成功进入启动 AVI 并播放 22050 Hz PCM。真机性能、所有片段的音画
同步及完整剧情仍需最终回归。
