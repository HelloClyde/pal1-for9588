...
转换为 RGB565 后逆时针转 90° 写入 9588 的 240×320 竖屏帧缓冲。
帧缓冲地址由 SDK 从 GUI API
...
线性重采样后与音乐混音。原版 VOC 中大量音效不是 22050 Hz 的整数分频，例如夜间
更鼓的 93 号音效为 6000 Hz；桌面默认的 SINC 重采样在 JZ4730 软浮点环境会阻塞主
线程，导致音效播放期间按键和触摸停顿。PCM 使用 2048-sample（约 92.9 ms）缓冲，
由按键轮询、整帧提交前后和延时函数共同泵入固件队列，每次最多提交四块，退出时调用
公开的 `bda_audio_stop()`。模拟器的片头重解压阶段仍可能产生短暂欠载，真机表现待测。
